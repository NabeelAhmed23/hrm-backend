...
export class ComplianceService {
  ...
  async getEmployeeCompliance(
    ...
    try {
      ...
      if (!['HR', 'ADMIN', 'SUPERADMIN'].includes(requestingUserRole)) {
        ...
        if (!requestingUserEmployee || requestingUserEmployee.id !== employeeId) {
          throw new AuthorizationError('You can only view your own compliance status');
        }
      }
      ...
      throw new InternalServerError('Failed to retrieve employee compliance status');
    }
  }
  ...
  async getOrganizationCompliance(
    ...
    try {
      ...
      if (!['HR', 'ADMIN', 'SUPERADMIN'].includes(requestingUserRole)) {
        throw new AuthorizationError('Only HR and ADMIN users can view organization compliance');
      }
      ...
      throw new InternalServerError('Failed to retrieve organization compliance status');
    }
  }
  ...
  async getComplianceByType(
    ...
    try {
      ...
      if (!['HR', 'ADMIN', 'SUPERADMIN'].includes(requestingUserRole)) {
        throw new AuthorizationError('Only HR and ADMIN users can view compliance by type');
      }
      ...
      const typeGroups = new Map<DocumentType, Array<{
        id: string;
        title: string;
        expiresAt: Date | null;
      }>>();
      ...
      typeGroups.forEach((docs, type) => {
        ...
        const summary = {
          ...
          yellow: documentCompliance.filter(d => d.status === 'YELLOW').length,
      ...
      complianceByType.sort((a, b) => a.documentType.localeCompare(b.documentType));
      ...
      throw new InternalServerError('Failed to retrieve compliance by document type');
    }
  }
   ...
   * @param organizationId - Organization ID
   * @param requestingUserRole - Role of requesting user (must be HR/ADMIN)
   * @returns List of employees with expired documents
   */
  async getCriticalComplianceIssues(
    ...
    try {
      // Authorization: Only HR and ADMIN can view critical issues
      if (!['HR', 'ADMIN', 'SUPERADMIN'].includes(requestingUserRole)) {
        throw new AuthorizationError('Only HR and ADMIN users can view critical compliance issues');
      }

      const now = new Date();

      // Fetch employees with expired documents
      ...
      throw new InternalServerError('Failed to retrieve critical compliance issues');
    }
  }
   ...
   * @param organizationId - Organization ID
   * @param requestingUserRole - Role of requesting user (must be HR/ADMIN)
   * @returns High-level compliance metrics
   */
  async getComplianceMetrics(
    ...
    try {
      // Authorization: Only HR and ADMIN can view metrics
      if (!['HR', 'ADMIN', 'SUPERADMIN'].includes(requestingUserRole)) {
        throw new AuthorizationError('Only HR and ADMIN users can view compliance metrics');
      }
      ...
      const orgCompliance = await this.getOrganizationCompliance(organizationId, requestingUserRole);

      // Count documents expiring soon and expired
      const documentCounts = await prisma.document.aggregate({
        where: {
          organizationId,
          deletedAt: null,
          employeeId: { not: null },
      ...
      const complianceRate = orgCompliance.summary.total > 0 
        ? Math.round((orgCompliance.summary.green / orgCompliance.summary.total) * 100)
        : 100;
